...
const Favarite = async () => {
  ...
  const favoriteDatas = datas.filter((workspace) => workspace.isFavorite);

  return (
    <div className="space-y-3 w-full">
      <div className="flex justify-between items-center px-8">
        <h2 className="font-semibold text-xl text-gray-500">Favorite</h2>
      ...
      <div className="h-[90%] overflow-y-scroll">
        {favoriteDatas.length > 0 ? (
          ...
          <p className="text-center text-gray-500 mt-4">No favorite workspaces found.</p>
        )}
      </div>
    </div>
  );
